...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  var logFile = null
  var logPath = repo.homeDir + '/log/' + process.env.NODE_ENV + '.log'
  ...
  var self = this,
      app  = repo.app,
      logPath = repo.homeDir + '/log/' + process.env.NODE_ENV + '.log',
      meta = { app: app.name, user: app.user, logFile: logFile },
  ...
  // Generate token
  var token
  if (app.name === "home" || app.name === "proxy" || app.name === "data-system") {
    token = haibu.config.get('authToken') || "";
  } else {
    token = app.password;
  }
  // Transmit application's name and token to drone
  spawnOptions.env.NAME = app.name;
  spawnOptions.env.TOKEN = token
  foreverOptions = {
    fork:      true,
    silent:    true,
    stdio:     [ 'ipc', 'pipe', 'pipe' ],
  ...
  fs.stat(repo.startScript, function (err, stats) {
    if (err) {
      var err = new Error('package.json error: ' + 'can\'t find starting script: ' + repo.startScript);
      err.blame = {
        type: 'user',
    ...
    function onChildStart (monitor, data) {
      ...
      //
      // Store application's pid in pidFile
      tmp = monitor.childData.cwd.split('/');
      name = tmp[tmp.length-2]
      monitor.childData.pidFile = path.join(haibu.config.get('directories:pid'),
            name + '.pid');
      if (!fs.existsSync(monitor.childData.pidFile)) {
        fs.openSync(monitor.childData.pidFile, 'w')
      }
      fs.chmod(monitor.childData.pidFile, 0700, function (err) {
        fs.writeFileSync(monitor.childData.pidFile, monitor.childData.pid);
      });
