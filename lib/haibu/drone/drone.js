...
Drone.prototype.stop = function (name, cleanup, callback) {
  ...
  //
  // Remove pidFile
  pidFile = path.join(haibu.config.get('directories:pid'), name + '.pid');
...
Drone.prototype._autostartUpdate = function (action, app, drones, callback) {
  ...
  function writeFile() {
    fs.writeFile(autostartFile, appJson, function(err){
      fs.chmodSync(autostartFile, 0700);
      callback(err);
    });
  }
...
Drone.prototype.buildBrunch = function(app, callback) {
  if (!app || !app.name) {
    return callback(false, new Error('`name` is required.'));
  }
  if (!app || !this.apps || !this.apps[app.name]) {
    return callback(false, new Error('Cannot update application that is not running.'));
  }

  callback = callback || function () {};
  drone = this.show(app.name);
  dirApp = drone.drones[0].spawnWith.cwd;
          ...
          try {
            ...
            haibu.common.npm.install(dirClient, app, function (err) {
              if (err) {
                ...
                // Build brunch
                //
                haibu.common.brunch.build(dirClient, app, function(err) {
                  return callback(true, err);
                });
              }
            });
...
//
// ### function lightUpdate (name, callback)
// #### @name {string} Name of the application to update (i.e. app.name).
// Git pull application if type of repository is git
// and build brunch if it is necessary.
//
Drone.prototype.lightUpdate = function (app, callback) {
  var self = this;
  if (!app || !app.name || !app.repository || !app.repository.type) {
    return callback(new Error('Both `name`, `repository.type` are required.'), false);
  }
  if (!app || !this.apps || !this.apps[app.name]) {
    return callback(new Error('Cannot update application that is not running.', false));
  }
  callback = callback || function () {};
  if (app.repository.type === "git") {
    self.stop(app.name, function (err, result) {
      //
      // Ignore errors and continue light-updating
      //
      ...
      if (app.repository.type === "git") {
        ...
        repo.update( function(err) {
          ...
          self.start(app, function (error, result) {
            if (err) {
