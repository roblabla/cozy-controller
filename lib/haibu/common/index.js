...
var brunch = common.brunch = require('./brunch');
var log = common.log = require('./log');

common.welcome = [
  '    ______  ______  ______  _    _      '.yellow,
  '   / ____/ / __  / /___  / //___//       '.yellow,
  '  / /     / / / /   ___//  \\_  _/  '.yellow,
  ' / /___  / /_/ / //____     / /   '.yellow,
  '/_____/ /_____/ /_____/    /_/ '.yellow,
  '  ',
  '  The Cozy Controller is Open Source Software available under'.grey,
  '  the MIT License.'.grey,
  '  ',
  ('  © ' + (new Date()).getFullYear() + ' Cozy Cloud').grey,
  ''
];
...
common.showWelcome = function (role, ipAddress, port) {
  ...
  dirApp = haibu.config.get('directories:apps');
  per = haibu.config.get('permissions');
  if (per != undefined) {
    console.log('    Application directories : '.grey + dirApp + ' with permissions : '.grey + per + '\n');
  } else {
    console.log('    Application directories : '.grey + dirApp);
  }
  console.log('    Run in '.grey + process.env.NODE_ENV + " environment\n".grey);
  datetime = new Date();
...
common.rmApp = function (appsDir, app, callback) {
  return rimraf(path.join(appsDir, app.user), callback);
};
