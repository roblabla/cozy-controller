...
exports.initLog = function() {
  ...
  serverStart.forEach(function(name){
    haibu.on(name, function (message, meta) {
      datetime = new Date();
      console.log("\n\n[" + datetime + "]\n" +"INITIALIZING Cozy-Controller ...".green +
        "\naction:autostart".bold)
    })    
  })
  ...
  var errNames = ['error:service' , 'error', 'drone:clean:warning',
    'npm:install:failure','npm:clean_cache:failure','brunch:build:failure', 'drone:cleanAll:warning',
    'git:clone:error']
