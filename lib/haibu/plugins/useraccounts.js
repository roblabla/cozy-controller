...
useraccounts.attach = function (options) {
  ...
  haibu.common.npm.install = function install(dir, target, callback) {
    ...
    function changePermissions(err) {
      ...
      async.waterfall([
        function(next) {
          spawn('id', ['-g', 'nogroup']).on('exit', function(exitcode) {
            next(null, exitcode);
          });
        },
        function(exitCode, next) {
          var group;
          if (exitCode == 0) {
            group = "nogroup";
          } else {
          ...
          spawn('chown', ['-R', user + ':nobody', appdir + '/' + target.name]).on('exit', function(exitCode) {
            next(null, exitCode);
          });
        },
        function (exitCode, next) {
          ...
          if (exitCode) {
            next(new Error('Unable to grab ownership for files'));
          }
          else {
        ...
        function(exitCode, next) {
          ...
          if (exitCode) {
            next(new Error('Unable to change permissions for files'));
          }
          else {
            next();
          }
