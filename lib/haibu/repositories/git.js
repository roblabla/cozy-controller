...
Git.prototype.init = function (callback) {
  ...
  commands = [
    'cd ' + self.appDir + ' && git clone --depth 1 ' + self.app.repository.url,
  ...
  function executeUntilEmpty() {
    ...
    timeout = setTimeout( function() {
      ...
      exec('sudo pkill -9 -f  \'git clone ' + self.app.repository.url + '\'')
      exec('sudo pkill -9 -f  \'git-remote-https origin ' + self.app.repository.url + '\'')
      haibu.emit('git:clone:error', 'error', {
    ...
    clone = exec(command, function (err, stdout, stderr) {
...
Git.prototype.update = function (callback) {
  ...
  if (self.app.repository.branch) {
    commands = [
      'cd ' + path.join(self.appDir, match[1]) + ' && git reset --hard ',
